...
int x25519_keypair(uint8_t out_public_value[32], uint8_t out_private_key[32]) {
    if (random_read(out_private_key, 32)) {
        return 1;
    }
    ...
    const uint8_t peer_public_value[32]) 
{
    ...
    return constant_time_compare(kZeros, out_shared_key, 32) != 0;
}
